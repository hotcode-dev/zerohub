...
- Signaling AsyncAPI spec is `server/api.yml` (version `3.1.0`)
- Protocol is WebSocket with protobuf payloads (`application/protobuf`)
- Keep the spec aligned with:
  - `server/pkg/handler/handler.go` endpoint routing
  - `proto/zerohub/v1/client_message.proto`
  - `proto/zerohub/v1/server_message.proto`
- Model protobuf `oneof` as AsyncAPI message payload `oneOf` variants
- Update AsyncAPI whenever endpoint paths, message fields, or oneof variants change
